...
modifier onlySeller(uint _nftId) {
    require(msg.sender == sellerOf[_nftId], "Only seller is allowed to do this action");
    _;
}
...
function listProperty(uint _nftId, uint _purchaseAmount) public {
    ...
    require(tokenOwner == msg.sender, "Only token owners are allowed to list");
...
function depositEarnest(uint _nftId) public payable {
    ...
    require(msg.value >= purchaseAmount[_nftId], "Insufficient funds deposited");
...
function finalizeSale(uint _nftId) public onlyBuyer(_nftId) {
    ...
    require(escrowDeposits[_nftId] >= purchaseAmount[_nftId], "Insufficient funds deposited");
